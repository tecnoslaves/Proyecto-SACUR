...
{Fore.YELLOW}║  |_____/ /__/    \__\ \______|  \_______/  |_|   \_\  {Fore.YELLOW}║
...
{Style.RESET_ALL}"""
    ...
    def crear_clave(self):
        ...
        # Generar salt aleatorio y derivar clave (en memoria)
        salt = secrets.token_bytes(32)
        derived_key = hashlib.pbkdf2_hmac('sha256', password.encode(), salt, 200000)

        # Generar un verificador (no revela la clave)
        verifier = hashlib.sha256(derived_key + b'v1').digest()

        # Guardar en archivo oculto SOLO el salt y el verificador (no la clave)
        key_data = base64.b64encode(salt).decode() + '\n' + base64.b64encode(verifier).decode()
        with open(self.key_file, 'w') as f:
            f.write(key_data)

        # Permisos restrictivos (600) en sistemas tipo Unix
        try:
            os.chmod(self.key_file, 0o600)
        except Exception:
            pass

        # En Windows marcar archivo oculto
        if os.name == 'nt':
            try:
        ...
        # Mantener la clave derivada en memoria (en base64) para uso inmediato.
        self.db_password = base64.b64encode(derived_key).decode()

        # Intenta eliminar la variable de contraseña en claro
        password = None
        confirm = None

        print("Clave de cifrado creada correctamente. NO se almacena la clave en disco.")
    ...
    def cargar_clave(self):
        ...
        try:
            with open(self.key_file, 'r') as f:
                ...
                stored_verifier = base64.b64decode(lines[1])
            ...
            # Mantener la clave derivada en memoria (no en disco)
            self.db_password = base64.b64encode(derived_key).decode()
            # eliminar variables sensibles en RAM
            password = None
            derived_key = None

            # Asegurar permisos del key_file por si alguien lo creó sin protección
            try:
                os.chmod(self.key_file, 0o600)
            except Exception:
                pass
    ...
    def crear_backup_cifrado(self, backup_path=None):
        """Crear un backup cifrado de la base de datos usando SQLCipher export.
        El backup estará protegido con la misma clave derivada en memoria.
        """
        if not self.db_password:
            print("No hay clave cargada en memoria. Inicie sesión primero.")
            return False

        if backup_path is None:
            ts = datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')
            backup_path = f"{os.path.splitext(self.db_path)[0]}_backup_{ts}.db"

        try:
            # Abrir la base de datos origen con la clave en memoria
            conn_src = sqlite3.connect(self.db_path)
            conn_src.execute(f"PRAGMA key = '{self.db_password}'")
            # Crear/adjuntar la base backup con la misma clave (se exporta cifrada)
            conn_src.execute(f"ATTACH DATABASE '{backup_path}' AS backup KEY '{self.db_password}';")
            conn_src.execute("SELECT sqlcipher_export('backup');")
            conn_src.execute("DETACH DATABASE backup;")
            conn_src.commit()
            conn_src.close()

            # Asegurar permisos restrictivos del backup
            try:
            ...
            print(f"Error creando backup cifrado: {e}")
            return False
    ...
    def cambiar_contraseña(self):
        ...
        try:
            ...
            # Generar nueva clave (no guardar la clave derivada en disco)
            new_salt = secrets.token_bytes(32)
            new_derived_key = hashlib.pbkdf2_hmac('sha256', new_password.encode(), new_salt, 200000)
            new_verifier = hashlib.sha256(new_derived_key + b'v1').digest()

            # Guardar sólo salt + verificador
            key_data = base64.b64encode(new_salt).decode() + '\n' + base64.b64encode(new_verifier).decode()
            with open(self.key_file, 'w') as f:
                f.write(key_data)
            ...
            # Mantener la nueva clave derivada en memoria
            self.db_password = base64.b64encode(new_derived_key).decode()
            # Eliminar variables sensibles
            new_password = None
            new_derived_key = None

            print("Contraseña cambiada exitosamente.")

        except FileNotFoundError:
            print("Error: Archivo de clave no encontrado.")
        except Exception as e:
        ...
        while True:
            ...
            try:
                ...
                if categoria == 1:
                    ...
                    resultado = self.seleccionar_productos("tragos", "Tragos")
                    if resultado == "finalizar":
                        break
                elif categoria == 8:
                    break
    ...
    def procesar_venta(self):
        ...
        while True:
            print("\nTipo de servicio:")
            print("1. Para llevar")
            print("2. Para consumir en el local")
